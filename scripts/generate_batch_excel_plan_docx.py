...
def build_doc() -> Document:
    ...
    add_para(
        doc,
        "Настроить пайплайн один раз (промпты, стили, генераторы, canvas). "
    ...
    add_para(
        doc,
        "Вывод: не писать с нуля, а доработать mass-lanes и добавить понятный вход через таблицу.",
        bold=True,
    )
    ...
    add_para(
        doc,
        "Используется topics.xlsx (лист «Темы»), как в Telegram-батче. "
    ...
    add_table(
        ...
        [
            [
                "A. Доработать mass-lanes",
                "Родитель-шаблон + N детей",
                "Мало риска, уже работает",
                "Много проектов в sidebar",
                "Да, v1",
            ],
            ["B. Один проект + смена строки", "current_row, reset", "Проще в списке", "Сложный reset", "Нет"],
            [
                "C. Объединить с TG BatchProject",
                "Одна модель БД",
                "Один код",
                "Большой рефакторинг",
                "Позже, v2",
    ...
    add_table(
        ...
        [
            [
                "A — только настройки",
                "Копируем промпты/vars, не картинки/видео",
                "30 роликов, разные темы, один стиль промптов",
            ],
            [
                "B — + общие ассеты",
                "Плюс один продукт/герой/референс",
                "Все ролики с одним продуктом в кадре",
            ],
        ],
    )
    add_bullets(
