...
cmd_backup() {
  ...
  if ! "${COMPOSE[@]}" ps --status running --services 2>/dev/null | grep -qx db; then
    echo "деплой: база не запущена — бэкап пропущен (первый запуск?)"
    return 0
  fi

  # Имя базы и суперпользователь нужны самой команде pg_dump, а не compose,
  # поэтому читаются здесь. ./env заполняет оператор, в git его нет —
  # статически проверять нечего.
  set -a
