...
Auth: Bearer KIE_API_KEY (фолбэк GPT_API_KEY на kie.ai), напрямую.
Адаптация под веб-версию: одиночный ключ из `kie_kling.kie_api_key()`
(без перебора ключей и VPS-relay — их нет в `kie_kling` этой ветки).
Образец: локалка, спека `docs/openspec/changes/kie-port/`.
"""
...
def _check(payload: Any, *, http_status: int, where: str) -> dict[str, Any]:
    ...
    if code_i == 401:
        logger.warning("kie 401 {} base={}", where, kie_api_base_url())
        msg += " — ключ KIE_API_KEY, проверь базу"
    raise KieHttpError(
...
async def upload_file(content: bytes, filename: str, *, upload_path: str = "images/studio-create") -> str:
...
async def create_task(api: str, endpoint: str | None, body: dict[str, Any]) -> str:
    ...
    if isinstance(raw_inner, dict):
        inner = raw_inner
    task_id = str(inner.get("taskId") or inner.get("task_id") or inner.get("id") or "").strip()
    if not task_id:
        raise KieHttpError(
...
async def kie_post_json(
    ...
    """POST JSON одиночным ключом (без перебора — см. шапку модуля)."""
    try:
        payload_body = dict(body)
        async with httpx.AsyncClient(timeout=timeout) as client:
            r = await client.post(url, headers=_headers(), json=payload_body)
        try:
            payload = r.json()
        except Exception:  # noqa: BLE001
            payload = {"msg": (r.text or "")[:300], "code": r.status_code}
        if not isinstance(payload, dict):
            payload = {"msg": str(payload)[:300], "code": r.status_code}
        return r, payload, "KIE_API_KEY"
    except KieHttpError:
        raise
    except Exception as e:  # noqa: BLE001
        raise KieHttpError(
            f"kie post {url.split('?')[0][-48:]}: {type(e).__name__}",
...
async def poll_task(api: str, task_id: str, *, timeout_s: float = _POLL_MAX_S) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        while True:
            try:
                r = await client.get(url, headers=_headers(), params={"taskId": task_id})
                net_fails = 0
...
async def generate_kie_video(
    ...
    *,
    ...
    # ByteDance Seedance first-frame I2V требует aspect_ratio='adaptive'
