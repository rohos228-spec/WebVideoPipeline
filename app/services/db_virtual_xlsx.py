...
async def build_virtual_project_sheets(session: AsyncSession, project: Project) -> dict[str, list[list[str]]]:
    """Генерирует словарь матриц {sheet_name: rows} напрямую из БД SQLite."""
    # 1. Загружаем кадры
    stmt = select(Frame).where(Frame.project_id == project.id).order_by(Frame.number)
    ...
    stmt_entities = select(Entity).where(Entity.project_id == project.id).order_by(Entity.id)
