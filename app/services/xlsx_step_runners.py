...
_PLAN_DB_HINT = (
    "\n\n# ЗАПИСЬ СЦЕНАРИЯ — ИСТОЧНИК ПРАВДЫ БАЗА\n"
    "Верни ТОЛЬКО JSON apply-ops:\n"
...
async def run_plan_xlsx(
    ...
    *,
    ...
    # Только инструкция. Книга раньше шла контекстом — для нового проекта это
    # пустой шаблон, а сопроводительный текст про «лист „Общий план“» сбивал
    # модель между таблицей и apply-ops. При учётных записях файла и так нет.
    attachments = [prompt_file]
...
_SPLIT_DB_HINT = (
    "\n\n# РАЗБИВКА — ИСТОЧНИК ПРАВДЫ БАЗА\n"
    "Верни ТОЛЬКО JSON apply-ops:\n"
    '{"ops":[{"target":"replace_frames","frames":['
    '{"закадр":"текст кадра 1","длительность":3},'
    '{"закадр":"текст кадра 2"}'
    "]}]}\n"
    "ФОРМАТ: если в сценарии (general_plan) или параметрах задано количество кадров — строго делай ровно это число кадров (соседние фразы одной сцены объединяй). "
    "Иначе для стандартного ролика 60–75 сек делай 10–20 кадров (максимум 30). "
    "Запрещено дробить на микро-фразы по 1-2 слова. "
    "Один кадр = законченная мысль/сцена. "
...
_IMG_PR_DB_HINT = (
    "\n\n# ПРОМТЫ КАРТИНОК — ИСТОЧНИК ПРАВДЫ БАЗА\n"
    "Верни ТОЛЬКО JSON apply-ops:\n"
