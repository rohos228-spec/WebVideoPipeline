...
def canonical_stitch(value: str) -> str:
    ...
    for sid, label in COVERAGE_STITCH_CHOICES:
        if text in (sid, label):
            return sid
        if text.casefold() in (sid.casefold(), label.casefold()):
            return sid
    return text
...
async def apply_coverage_op(
    ...
    if report is not None:
        ...
        if isinstance(renumber, dict) and renumber:
            renumber_map: dict[int, int] = {int(k): int(v) for k, v in renumber.items()}
            out["renumber"] = renumber_map
