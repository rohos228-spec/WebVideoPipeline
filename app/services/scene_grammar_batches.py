...
акцент (уникальный объект/жест), действие, описание_кадра (камера+композиция+фаза действия),
логика_перехода, фон (подробно: поверхности/глубина/цвет), освещение, предметы.

=== ЦЕПЬ ДЕЙСТВИЯ (КРИТИЧНО, v1.8) ===
Перед shots напиши сквозную цепь фаз сцены (3–7 глаголов). Каждый shot = одна фаза.
Каждый кадр = mid-action freeze (тело/рука/предмет В ДВИЖЕНИИ), НЕ статичная поза.
БРАК-действия без движения: стоит/сидит/лежит/смотрит/замер/находится/позирует/замирают.
Соседние shots одной сцены отличаются ≥2 из: фаза действия, поза тела, состояние пропа,
крупность/ось. Визуальный twin (тот же силуэт+prop+дистанция) = брак — перепиши.
описание_кадра обязательно говорит ДЕЛЬТУ от прошлого шота («теперь рука уже…»).

Непрерывность мира: соседние shots в том же месте → тот же базовый фон и то же
освещение. Действие ОБЯЗАНО продвинуться (фаза N→N+1); «тот же жест другим
ракурсом» = брак. Чередуй крупности (общий/средний/CU/портрет/деталь).
Запрещено: одинаковый акцент; абстрактный смысл; «интерьер» в одно слово;
разный свет в одном помещении без смены времени/места;
Детективная доска два раза подряд.

КРИТИЧНО — ОТВЕТ:
Верни ТОЛЬКО JSON (без прозы, без планов «проверю xlsx», без вложений):
{{"characters":[],"scenes":[...только эти id_scene, полный паспорт + shots...],"ops":[],"report":"..."}}
Excel/.xlsx НЕ существует в этом шаге. Пайплайн сам пишет JSON в DB.
Не трогай другие сцены. Не пиши error про объём / «нет xlsx».
Без markdown.
""".strip()
