...
def _slug(value: str, limit: int = 48) -> str:
    """Имя контракта → безопасный кусок имени файла.

    Точка в алфавите разрешена (имена вроде `vp.v2` законны), и одного её
    разрешения хватает, чтобы `../../` превратилось в `..-..`: в путь это уже
    не вырвется, но имя файла с `..` в начале — приглашение к следующей
    ошибке. Поэтому точки по краям снимаются, а их серии схлопываются.
    """
