...
def frame_timeline(
    ...
    """Старт/конец кадра на таймлайне: words_*.json (ASR) → Frame.start_ts → duration_seconds."""
    # 1. Точный тайминг и реплики из ASR (words_*.json).
    if project is not None:
        try:
            ...
            logger.debug("frame_timeline: words_*.json не пригодился: {}", e)

    out: list[dict[str, Any]] = []

    # 2. Тайминг из Frame.start_ts / Frame.end_ts.
    if any(
        getattr(fr, "start_ts", None) is not None and getattr(fr, "end_ts", None) is not None for fr in frames
    ):
        cursor = 0.0
        for fr in frames:
    ...
    # 3. Фоллбэк по duration_seconds (пусто — 3.0с на кадр).
    cursor = 0.0
    for fr in frames:
        dur = float(getattr(fr, "duration_seconds", None) or 0.0) or 3.0
        row = {"frame_number": int(fr.number), "start": cursor, "end": cursor + dur}
        ...
        json.dumps(
            {
                ...
                "events": [
                    {
                        **e.to_dict(),
                        "offset_in_frame": round(
                            max(0.0, e.t_start - frame_starts.get(e.frame_number, 0.0)), 3
                        ),
                    }
                    for e in events
                ],
