...
6. РЕЖИМ ТРАНСЛЯЦИИ ГОТОВОГО ПРОМПТА В JSON-АГЕНТА: Если пользователь передал уже готовый подробный авторский промпт/инструкцию агента — сохрани 100% его авторской логики, художественных правил, стилистики и ролевых установок. Твоя ключевая задача в этом случае — безупречно перевести и упаковать авторский промпт в строгий формат JSON-агента целевой ноды с сохранением схемы полей и правил валидации пайплайна.
"""
...
async def compile_meta_prompt(
    ...
    *,
    ...
    guideline = NODE_SPECIFIC_GUIDELINES.get(normalized_step, NODE_SPECIFIC_GUIDELINES.get("excel_gpt", ""))
...
"""
    ...
    gpt = get_gpt_client()
    logger.info(
        "meta_agent: компиляция промпта для step={} intent='{}'...", normalized_step, clean_intent[:50]
    )
    ...
    has_guards = any(
        w in compiled_text.lower() for w in ["strict", "constraint", "no-chat", "запрещено", "only"]
    )
