...
def _plan_rows_by_scene(
    ...
    for rows in out.values():
        rows.sort(key=lambda r: int(r.get("phase_index") or r.get("shot_index") or 0))
    return out
...
def _shot_move(frame: dict[str, Any], overlay: dict[str, Any] | None = None) -> str:
    ...
    cs = _camera_subdivide(frame)
    notes = frame_field(frame, "shot01_notes")
    return _text(cs.get("движение"), cs.get("move")) or _token_from(notes, _MOVE_TOKENS)
...
def _shot_dto(
    ...
    *,
    ...
    who = _text(ov.get("кто_в_кадре"), ov.get("персонажи")) or frame_field(frame, "characters", "персонажи")
    ...
    fr = (
        (
            await session.execute(
                select(Frame).where(Frame.project_id == project_id, Frame.uuid == parent_uuid)
            )
        )
        .scalars()
        .first()
...
async def edit_shot_field(
    ...
    fr = (
        (await session.execute(select(Frame).where(Frame.project_id == project_id, Frame.uuid == frame_uuid)))
        .scalars()
        .first()
    )
    if fr is None:
...
async def add_cell(
    ...
    *,
    ...
    if before_index is None or before_index > len(cells):
        ...
        before_sk = float(before_frame.sort_key or 0.0) if before_frame is not None else None
