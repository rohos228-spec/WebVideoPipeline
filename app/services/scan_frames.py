...
async def scan_missing_frames(session: AsyncSession, project: Project) -> list[int]:
    ...
    logger.info(
        "[#{}] scan_missing_frames: всего кадров={}, с image_prompt={}, недостающих={}, scenes_dir={}",
        project.id,
        len(frames),
        total_with_prompt,
        len(missing),
...
async def scan_missing_shot2_frames(session: AsyncSession, project: Project) -> list[int]:
    ...
    for fr in frames:
        ...
        if not disk_has_valid_shot2_image(scenes_dir, fr.number):
            missing.append(fr.number)
    logger.info(
        "[#{}] scan_missing_shot2_frames: shot_02 ожидается={}, без PNG на диске={}, scenes_dir={}",
        project.id,
        expected,
        len(missing),
        scenes_dir,
...
async def sync_frames_with_disk_images(session: AsyncSession, project: Project) -> int:
    ...
    rows = (
        (
            await session.execute(
                select(Frame).where(Frame.project_id == project.id).where(Frame.number.in_(numbers))
            )
        )
        .scalars()
        .all()
...
async def reset_shot2_to_prompt_ready(session: AsyncSession, project: Project, numbers: list[int]) -> int:
...
def _disk_has_frame_video_shot1(videos_dir: Path, frame_number: int) -> bool:
    ...
    if not videos_dir.exists():
        return False
    return any(p for p in videos_dir.glob(f"clip_{frame_number:03d}_*.mp4") if "_s2_" not in p.name)
...
async def scan_missing_videos_shot1(session: AsyncSession, project: Project) -> list[int]:
...
async def scan_missing_shot2_videos(session: AsyncSession, project: Project) -> list[int]:
    ...
    for fr in frames:
        ...
        if not disk_has_shot2_video(videos_dir, fr.number):
            missing.append(fr.number)
    logger.info(
        "[#{}] scan_missing_shot2_videos: shot_02 ожидается={}, без clip s2={}, videos_dir={}",
        project.id,
        expected,
        len(missing),
        videos_dir,
...
async def scan_missing_videos(session: AsyncSession, project: Project) -> list[int]:
...
async def reset_shot2_for_video_regen(session: AsyncSession, project: Project, numbers: list[int]) -> int:
...
async def reset_frames_for_video_regen(session: AsyncSession, project: Project, numbers: list[int]) -> int:
