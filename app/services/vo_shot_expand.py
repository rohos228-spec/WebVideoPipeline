...
def with_character_sheet_lock(prompt: str, sheet_ids: list[str]) -> str:
    """Image N = какой cXX. Без этого image-модель сливает два листа в одно лицо."""
    raw = (prompt or "").strip()
    ids = [str(x).strip() for x in sheet_ids if str(x).strip()]
    if not ids:
        return raw
    if raw.startswith(_IDENTITY_LOCK_PREFIX) or raw.startswith("Image 1 is the previous coverage still"):
        return raw
    lines: list[str] = []
    ...
    if len(ids) >= 2:
        ...
        lines.append(f"Exactly one body of {ids[0]}. Do not copy the sheet grid or pose.")
    return "\n".join(lines) + "\n\n" + raw
