...
PROVIDER_BATCH_CHAR_BUDGET: dict[str, int] = {
    "minimax": 40_000,
    # Измерено живым прогоном 2026-08-31: Opus через vibecode тянет ответ
    # ~94k символов и на этой границе недетерминирован — 33 кадра × 4000
    # уходили одним вызовом, и один и тот же запрос то укладывался
    # (93 916 симв, finish=stop), то обрезался. Три обрыва по ~12 минут
    # подряд, четвёртая попытка прошла; штатное дробление так и не
    # понадобилось, то есть исход зависел от `GPT_MAX_RETRIES`, а не от
    # плана. 64k — заметно ниже границы, а не впритык к ней: 33 кадра
    # становятся тремя вызовами по 11.
    "vibecode": 64_000,
}
