    ...
    if fr_total == 0:
        ...
        if _enrich_meta_allowed_for_status(project):
            ...
            if enrich_st is not None:
                return enrich_st
            cur = getattr(project, "status", None)
            # Только enrich-полка: держать images_ready/videos_ready при нуле
            # кадров нечем — так reset split возвращал старый статус вместо
            # script_ready (tests/test_reset_step.py).
            if (
                cur is not None
                and _status_ord(ProjectStatus.enrich_1_ready)
                <= _status_ord(cur)
                <= _status_ord(ProjectStatus.enrich_5_ready)
...
async def recompute_all(
    ...
    `skip_assembled` — не трогать досчитанные проекты (на старте студии их
    пересчёт ничего не меняет, но стоит полного обхода артефактов).
    """
    stmt = select(Project)
    if skip_assembled:
