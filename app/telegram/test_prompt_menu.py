...
def test_root_kb(projects: list[TestPromptProject]) -> InlineKeyboardMarkup:
    ...
    for p in projects:
        rows.append(
            [
                InlineKeyboardButton(
                    text=(f"#{p.id} {p.name[:40]} · iter={p.current_iter} · {p.status}"),
...
def test_project_kb(p: TestPromptProject) -> InlineKeyboardMarkup:
    ...
    rows.append(
        [
            InlineKeyboardButton(
                text=(f"📝 Визуальный промт {'✅' if has_visual else '❌'}"),
    ...
    rows.append(
        [
            InlineKeyboardButton(
                text=(f"⚙ Системный промт для GPT {'✅' if has_system else '❌'}"),
    ...
    if busy:
        ...
        if has_visual and has_system:
            label = "▶ Поехали" if p.current_iter == 0 else "▶ Повторить с теми же промтами"
            rows.append(
                [
                    InlineKeyboardButton(
                        text=label,
