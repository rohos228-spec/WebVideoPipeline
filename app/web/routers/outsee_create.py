...
def _smart_enhance_prompt(text: str, style: str | None = None) -> str:
    ...
    if any(ch in "abcdefghijklmnopqrstuvwxyz" for ch in lower) and not any(
        ch in "абвгдеёжзийклмнопрстуфхцчшщъыьэюя" for ch in lower
    ):
...
async def enhance_prompt_endpoint(req: EnhancePromptRequest) -> dict[str, Any]:
    ...
    # kie.ai как второй LLM для enhancer у нас не переносится (решение
    # владельца 2026-09-03): без ключа ветка была мёртвой, модуля kie_http нет.

    # 2. Умный локальный генератор (если ключи LLM не заданы или при сбое)
