...
def render_html_report(analysis: dict, *, title: str = "Отчёт мониторинга video-pipeline") -> str:
    ...
    timing_tbody = (
        "\n".join(timing_rows)
        if timing_rows
        else "<tr><td colspan='6' class='empty'>Нет данных о таймингах</td></tr>"
    )
    ...
    errors_tbody = (
        "\n".join(error_rows)
        if error_rows
        else "<tr><td colspan='5' class='empty' style='color:#4ade80;'>Ошибок не обнаружено!</td></tr>"
    )

    events_rows = []
    for ev, cnt in list(event_counts.items())[:20]:
        events_rows.append(f"<tr><td><code>{_html.escape(ev)}</code></td><td>{cnt}</td></tr>")
    events_tbody = (
