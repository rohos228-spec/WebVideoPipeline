...
class ProjectRoute(Base):
    """Кому принадлежит проект и в каком он статусе. Вне политик RLS.
    ...
    сканирование межарендно, а RLS запрещает межарендное чтение, в том и
    смысл. Воркер, не назначивший арендатора, видит проекты владельца и ни
    одного клиентского: не ошибка, а тишина.

    Отсюда разделение. Здесь лежат **маршрутные факты и только они** —
    номер проекта, его арендатор, его статус. Ни темы, ни текстов, ни
    промтов, ни цен: всё содержимое остаётся в `projects` под политикой.
    Воркер читает эту таблицу без арендатора, узнаёт, у кого есть работа, и
    дальше делает её уже под нужным арендатором через `tenant_scope`.

    **Синхронность обеспечивает триггер, а не приложение.** Статус проекта
    меняют десятки мест, часть из них — массовыми `update()` мимо ORM.
    Список «не забыть обновить маршрут» разошёлся бы с реальностью на первой
    же забытой строке, и разошёлся бы молча: воркер просто перестал бы
    видеть чей-то проект. Поэтому зеркалирование висит триггером на
    `projects` (ревизия 0007) — обойти его из приложения нельзя.

    **Политика у таблицы своя, обратная обычной** (`tenant_routes`, 0007):
    сессия без арендатора видит все строки, сессия с арендатором — только
    свои. У остальных таблиц незаданный арендатор означает «видно только
    ничьё», и это правильно: забытая привязка не должна становиться
    утечкой. Здесь наоборот, потому что режим без арендатора и есть
    служебный режим воркера — иначе он не найдёт работу вовсе. Цена
    асимметрии ограничена тем, что в таблице нет содержимого.
    """

    __tablename__ = "project_routes"
    __table_args__ = (Index("ix_project_routes_tenant_status", "tenant_id", "status_value"),)

    project_id: Mapped[int] = mapped_column(primary_key=True)
    # NULL — проект владельца, заведённый до перехода в SaaS.
    tenant_id: Mapped[str | None] = mapped_column(Uuid(as_uuid=False), default=None, index=True)
    # Строка, а не enum: значение зеркалит триггер, и он не должен знать
    # ничего о питоновском перечислении. Фильтр по рабочим статусам делает
    # вызывающий, передавая список — так список остаётся один.
    status_value: Mapped[str] = mapped_column(String(40), index=True)
    updated_at: Mapped[datetime] = mapped_column(default=_now, onupdate=_now)
