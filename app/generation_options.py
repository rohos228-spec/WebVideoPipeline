...
# ---- Слоты референса на одну генерацию -------------------------------------
#
# Сколько картинок-референсов провайдер принимает за раз. Это СВОЙСТВО
# ПРОВАЙДЕРА, а не константа кода: правило «ровно одна фотография на кадр»,
# на котором построен `frame_cast.py` и раздел «Фотография одна» в мастер-
# промте картинок, писалось под MiniMax `image-01` — а его в контуре больше
# нет. Действующий Outsee берёт два, и на живом прогоне 2026-08-31 в кадр
# уходило `refs=2`. Разница не косметическая: по этому правилу оценивали
# возможности пайплайна («сцена на двоих недоступна, второй герой уедет за
# первым»), и оценка была занижена — ограничение начинается с ТРЕТЬЕГО
# персонажа.
...
#: лишний реф провайдер отвергает запросом целиком, недостающий лечится
#: текстовым описанием, которое промт и так требует для всех, кого назвал
#: `describe_appearance`.
DEFAULT_REF_SLOTS = 1


def ref_slots_for_provider(provider: str | None) -> int:
    """Сколько референсов принимает провайдер картинок за одну генерацию."""
    key = str(provider or "").strip().lower()
    return PROVIDER_REF_SLOTS.get(key, DEFAULT_REF_SLOTS)
