...
Гейт бежит по `prompts/` на диске — по тому, что есть. У владельца это вся
библиотека (59 файлов); в CI — только четыре папки, которые лежат в git
(`04_hero`, `05_excel_gpt`, `05_image_prompts`, `scene_design`, 28 файлов),
и среди них та самая `05_excel_gpt`, с которой всё началось. Первая
редакция требовала «больше 30 файлов» и уронила релиз #12 ровно на этом:
неполная библиотека — не ошибка, а норма чистого чекаута. Пропуск — только
при нуле файлов. На сервере ту же проверку делает предупреждение на старте
по базе.
...
def test_library_matches_the_contract() -> None:
    ...
    if not files:
        pytest.skip("в prompts/ нет ни одного файла промта — проверять нечего")
