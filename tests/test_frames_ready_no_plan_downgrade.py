...
async def test_ready_confirmed_frames_ready_keeps_current_status() -> None:
    ...
    ok = await ready_status_confirmed_by_data(_session_with_frame_count(3), p, ProjectStatus.frames_ready)
    assert ok is True
...
async def test_clamp_never_downgrades_frames_ready_to_new(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.services.step_data_guard.compute_actual_status", _fake_compute)
    monkeypatch.setattr("app.services.project_state.compute_actual_status", _fake_compute)
