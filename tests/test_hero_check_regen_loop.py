...
def test_start_regen_loop_sets_meta(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    p.meta = {
        "canvas_graph": {
            ...
            "edges": [{"source": hero_key, "target": check_key, "data": {"kind": "after"}}],
        },
    ...
    started = asyncio.run(hcr.maybe_start_hero_check_regen_after_check(_sess(), p, check_key))
...
def test_start_regen_skips_non_hero_upstream(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    started = asyncio.run(hcr.maybe_start_hero_check_regen_after_check(_sess(), p, "n_check"))
    assert started is False


def test_max_rounds_pauses_with_reason(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    out = upload_dir(p, check_key)
    out.mkdir(parents=True)
    (out / "check_report.txt").write_text("regen: c01\nverdict: fail\n", encoding="utf-8")
...
def test_return_to_check_after_hero(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    check_key = "n_check"
    ...
    nxt = asyncio.run(hcr.maybe_return_to_check_after_hero(_sess(), p))
...
def test_pass_clears_loop_meta(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
