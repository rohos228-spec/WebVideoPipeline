...
def test_empty_vo_shots_attach_to_parent_cell() -> None:
    ...
    assert child_vo != "—"
    joined = " ".join(s["voiceover_in_shot"] for s in cells[0]["shots"] if s["voiceover_in_shot"] != "—")
    assert " ".join(["Что,", "если", "самый", "страшный", "адрес"]) == " ".join(joined.split())
...
async def test_edit_cell_and_field_and_add_cell(mem_db) -> None:
    ...
    async with mem_db() as session:
        ...
        fr = (await session.execute(select(Frame).where(Frame.uuid == "u001"))).scalars().first()
        ...
        frames = list(
            (
                await session.execute(
                    select(Frame).where(Frame.project_id == p.id).order_by(Frame.sort_key, Frame.number)
                )
            )
            .scalars()
            .all()
...
def test_shot_plan_overlay_set_and_stitch_differ_inside_scene() -> None:
    ...
    assert " ".join(["Что", "если", "самый", "страшный", "адрес", "выглядел", "как", "ссора"]) == " ".join(
        " ".join(vos).split()
    )
