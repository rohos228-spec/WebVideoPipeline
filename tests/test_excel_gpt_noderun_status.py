...
@pytest.mark.asyncio
async def test_sync_heals_overflow_pending_completed_to_done(mem_db, monkeypatch) -> None:
        ...
        nodes = [
            {
                ...
                "type": "excel_gpt",
                "position": {"x": 0, "y": 0},
                "data": {"label": "GPT", "slotOverflow": True},
            },
            {
                "id": nxt,
                "type": "excel_gpt",
                "position": {"x": 290, "y": 0},
                "data": {"label": "Проверка", "slotOverflow": True},
            },
        ]
        wf = Workflow(
            name=f"wf-{uuid.uuid4().hex[:8]}",
