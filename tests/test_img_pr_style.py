...
def test_wrap_is_noop_regardless_of_style_id() -> None:
    ...
    assert wrap_scene_with_style(SCENE, style_block="STYLE: custom.\n\nFinal style lock: custom.") == SCENE
...
def test_resolve_project_img_style_from_variant() -> None:
    ...
    project = SimpleNamespace(meta={"prompt_slot_variants": {"n_image_prompts": {"main": "knitted_v1"}}})
    assert resolve_project_img_style(project) == "knitted"
