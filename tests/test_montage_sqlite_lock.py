...
def test_prepare_ai_change_retries_sqlite_transient() -> None:
    ...
    assert apply_mod._is_sqlite_transient(RuntimeError("database is locked"))
    assert apply_mod._is_sqlite_transient(
        RuntimeError("This Session's transaction has been rolled back due to a previous exception")
    )
...
def test_db_busy_timeout_at_least_60s() -> None:
    ...
    assert '"timeout": 60' in inspect.getsource(db_mod) or 'timeout": 60' in inspect.getsource(db_mod)
