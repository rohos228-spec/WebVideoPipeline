...
def test_response_looks_like_file_xlsx_url() -> None:
    assert _response_looks_like_file(_FakeResp("https://chatgpt.com/backend-api/files/abc/download"))


def test_response_looks_like_file_octet_stream() -> None:
    assert _response_looks_like_file(_FakeResp("https://x.com/dl", "application/octet-stream"))
...
def test_backend_file_url_variants_simple_to_download() -> None:
    url = "https://chatgpt.com/backend-api/files/file_abc/simple?conversation_id=x"
