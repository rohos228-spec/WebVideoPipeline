...
- The app connects to Chrome via CDP on `localhost:29229` for browser automation (Outsee CDP fallback, ElevenLabs). GPT **text** is API-only; default provider is **vibecode** (`VIBECODE_API_KEY`, `app/services/vibecode_catalog.py`), kie (`GPT_API_KEY`) — по явному выбору ноды; kie-ключ на vibecode не подставлять (`gpt_api.py:186-196`). **Outsee media** via Developer API: `OUTSEE_API_KEY` (profile outsee.io) → Bearer `https://outsee.io/api/v1/...` (`app/bots/outsee_http.py`). Set `IMAGE_PROVIDER=outsee` / `VIDEO_PROVIDER=outsee`. Fallback CDP: `OUTSEE_HTTP_FALLBACK_CDP=true`. Alternative: `IMAGE_PROVIDER=grsai` / `VIDEO_PROVIDER=grsai` (`GRSAI_API_KEY` — другой ключ).
...
- **Browser ChatGPT retired for text**: all text/xlsx/check/verdict/anim_pr/hero-prompt/music-prompt go through `app/services/gpt_client.py` → `gpt_api`. CDP remains only for Outsee (image/video/music gen) and ElevenLabs TTS. Default text provider — vibecode (`VIBECODE_API_KEY`, model `gpt-5.6-sol`); kie needs `GPT_API_KEY` + `GPT_BASE_URL` (`https://api.kie.ai`, path `/codex/v1/responses`, model `gpt-5-6-sol`).
