...
- **Два графа, и исполняется только один.** `Workflow` (`/api/workflows`) — шаблон для новых роликов; ролик идёт по `project.meta.canvas_graph`. Править граф ролика — только через `app/services/project_graph.py` (`/api/projects/{id}/graph`: diff → apply; выключенные узлы = `data.disabled`, из них выводится `meta.disabled_nodes`). Стадии (`pipeline_stages.py`) — представление этого графа: стадия без включённых узлов = `skipped`. Чат-агент (`/api/chat`) граф **предлагает** (`editGraph`/`proposeGraph` → `meta.graph_proposal`), применяет только `applyGraph confirm=true` после того, как человек увидел диф и список сгорающих шагов (`docs/SAAS-PIVOT.md` §8.2, уточнение 2026-08-26). Оркестратор из `db_browser.py` с `edit_files`/`git_commit_push` в UI не подключён намеренно.
...
- **CI/CD: GitHub Actions → GHCR → VPS.** `.github/workflows/ci.yml` — гейт (зеркало `.claude/verify.json`, плюс RLS на живом Postgres, который локально пропускается). `release.yml` — образ в ghcr.io и ssh-выкладка **дайджестом**, а не тегом `latest`: тег к моменту выкладки может указывать уже на следующую сборку. Деплой ждёт healthcheck (`up --wait`) и проверяет `/api/health` снаружи — «команда прошла» и «студия работает» это разные вещи.
- **Postgres на VPS живёт НА ХОСТЕ, не в контейнере** (решение владельца 2026-08-25): `deploy/studio/setup-postgres.sh`. Контейнер ходит к нему через смонтированный unix-сокет — TCP у базы выключен, снаружи её не видно. В `DATABASE_URL` параметр `?host=` — путь ВНУТРИ контейнера (всегда `/var/run/postgresql`), а `POSTGRES_SOCKET_DIR` — путь на хосте; перепутать легко, ошибка тихая. Бэкап снимается `sudo -u postgres pg_dump`: под ролью приложения он падает, потому что `FORCE ROW LEVEL SECURITY` действует и на владельца таблиц.
- **В образ НЕ входят `prompts/` и Playwright.** Библиотека промтов намеренно вне git и монтируется томом (`deploy/studio/docker-compose.yml`), на первом старте импортируется в базу, дальше источник — `prompt_library`. Chrome/CDP на VPS не запустится в принципе: нужен живой профиль с логинами. Поэтому там только API-провайдеры и `OUTSEE_HTTP_FALLBACK_CDP=false`.
- **`docker compose` в `deploy/studio` — только с `--env-file ./env`.** Файл назван `env`, а не `.env`, намеренно; `env_file:` внутри службы отдаёт переменные контейнеру и в подстановке `${…}` не участвует. Без флага compose отказывается собирать конфиг.
...
- **Claude на vibecode** (`TEXT_LLM_PROVIDER=vibecode`, каталог `app/services/text_llm_catalog.py`: Claude Opus 5 — дефолт, Claude Sonnet 5, GPT 5.6 Sol / 5.5). Шлюз отдаёт `claude-*` **только на `/v1/messages`** (формат Anthropic) — транспорт `app/services/anthropic_messages.py` на официальном SDK `anthropic` с `base_url` шлюза; `gpt_api._anthropic_route()` переключает по имени модели. `temperature` на Opus 5 / Sonnet 5 не передаётся (400). Haiku 4.5 и Fable 5 в каталог не заведены: Haiku отдаёт датированный id (ловит `_check_served_model`) и заворачивает JSON в ```json, Fable на vibecode — 502.
- **MiniMax** (`MINIMAX_API_KEY`, `app/bots/minimax.py`) — только **медиа**: `IMAGE_PROVIDER=minimax` (`image-01`, 9:16 → 720×1280), `VIDEO_PROVIDER=minimax` (Hailuo, submit→poll→retrieve). Стартовый кадр и реф уходят **base64 data URL** — публиковать кадры наружу не нужно. Текстовый контур MiniMax (`TEXT_LLM_PROVIDER=minimax`, `MiniMax-M3`) выведен 2026-08-26 решением владельца: модель не соблюдала structured outputs и врала на расплывчатых запросах.
