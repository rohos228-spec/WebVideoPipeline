...
> ⚠️ **ДОКУМЕНТ УСТАРЕЛ** (сверка с кодом 2026-08-20, см.
> `docs/openspec/docs-inventory.md`). Нормативные утверждения ниже
> противоречат коду: ветки — per-ПК через `ORCHESTRATOR_GIT_BRANCH`
> (`AGENTS.md`), текстовые LLM давно API-only (`app/services/gpt_api.py`),
> политика ретраев — `app/services/step_failure_policy.py` (никакого
> MAX_FAIL=3 → failed). Актуальная точка входа: `ai-pack/START_HERE.md`.
...
**Владелец и тестер** — один человек (chat_id владельца — в `.env`:
`TELEGRAM_OWNER_CHAT_ID`; имя бота — у владельца, здесь не публикуются).
...
- **SQLite** в `data/state.db`. Все стейты пайплайна, кадры, HITL-запросы,
  артефакты — там.
- **Telegram** — через **SOCKS5** (`socks5://user:pass@host:port`, реальное значение — в `.env` → `TELEGRAM_PROXY_URL`),
