...
- [ ] C.1 `app/models.py`: модель `LlmCall` (см. колонки в proposal §5;
...
- [ ] C.2 `app/services/llm_prices.json`: {model → input/output_usd_per_m},
...
- [ ] C.3 `app/services/llm_ledger.py` (каркас): `record(...)` —
...
- [ ] C.4 Юнит-тесты: расчёт цены (в т.ч. unknown model = 0 + warning;
...
- [ ] A.1 logical_call_id: contextvar в llm_ledger; ставится на входе
...
- [ ] A.2 Запись в 3 HTTP-точках — try/finally вокруг ВСЕГО тела
      ...
      из результата.
- [ ] A.3 Accounting-контекст: contextvar (project_id, node_key) в
...
- [ ] A.4 Тесты (мок httpx, как тесты транспорта этапа 5): каждый
...
- [ ] B.1 `sum_usage(*usages)` в gpt_api (или llm_ledger): покомпонентно
      prompt/completion/total, отсутствующие = 0; все пустые → {}.
- [ ] B.2 Правки `usage=first.usage` → сумма: `_chat_packed_parallel`
      (`:2035`), `_chat_adaptive_1_2_4` (`:2151`); continuation — 
      накопление sum(result.usage, cont.usage) по кругам (`:2349`,
      `:2455`).
- [ ] B.3 volume-добор: `volume_complete_apply_ops_reply`
...
- [ ] D.1 `llm_ledger.bind_prompt_hash(...)` (contextvar) + биндинг в 4
      ...
      изобретаем).
- [ ] D.2 `capture_attempt()` в llm_ledger — contextvar с
      ...
      (другой учёт, этап 5).
- [ ] D.3 Тесты: запись несёт хэш из биндинга; fallback работает;
...
- [ ] E.1 `app/settings.py`: `llm_budget_usd` (env `LLM_BUDGET_USD`,
...
- [ ] E.2 Проверка на входе внешнего `chat()` ДО платного вызова И
...
- [ ] E.2a Re-raise BudgetExhausted в контурах с широким except
...
- [ ] E.3 `step_failure_policy.on_step_failure`: ранняя ветка
...
- [ ] E.4 `POST /api/projects/{id}/llm-budget {budget_usd}` — meta
...
- [ ] E.5 API дашборда `app/web/routers/llm_costs.py`:
...
- [ ] E.6 Студия: страница `web/src/app/costs/` — таблица нод, сумма
      прогона, график разбивки по нодам, график динамики по прогонам,
      поле бюджета (E.4). ОДИН дашборд — смета; сверх — роадмап.
- [ ] E.7 Тесты: превышение бюджета → BudgetExhausted до вызова и
