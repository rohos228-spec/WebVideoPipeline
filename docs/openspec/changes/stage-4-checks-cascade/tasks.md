...
- [ ] D.1 Гейт-конфликт → fail: перевернуть ветку «keep gate=pass»
...
- [ ] D.2 [ok] не навсегда: `mark_ok_tokens_from_reply` → только при
...
- [ ] D.3 Убрать `mark_passed_except_regen` (неупомянутый = принят):
...
- [ ] D.4 `filter_image_paths_for_recheck`: recheck-набор = все кадры
...
- [ ] D.5 Severity-only отчёт ≠ pass, два слоя [панель 3/3]:
...
- [ ] D.6 Regen-цели без ложных срабатываний: заменить голый
...
- [ ] B.4 Доставка по kind: scenes shot2 → поле shot2-промпта; videos →
...
- [ ] B.5 Noop-regen детект: снимок промптов regen-целей в meta петли;
      байт-в-байт совпадение на следующем круге → `logger.error` +
      `vision_noop_regen` счётчик (регенерация выполняется, событие
      громкое).
- [ ] B.6 Тесты: каждая ось рожает правку промпта; shot2/hero/videos
...
- [ ] A.4 Решения оператора v1 — явный endpoint
...
- [ ] A.5 Тесты: исчерпание паузит с причиной; счётчик переживает
      исчерпание и рестарт (meta); решения оператора сбрасывают/
      принимают корректно.

## C. media_probe перед vision

- [ ] C.1 `media_probe.py`: `probe_image(path, expect_aspect)` (PIL:
...
- [ ] C.2 `video_sheet.py`: `_probe_duration_sec` удалить → 
      `media_probe.probe_duration`; ошибка пробы = исключение (не 8.0);
      `probe_video` до нарезки стиллов (§9#10).
- [ ] C.3 Check-препроцесс: `preflight_media_for_check` в checkMode
...
- [ ] C.4 Приёмка генерации: `probe_image` в `generate_images`,
      ...
      [панель 1/3, блокер].
- [ ] C.5 Тесты: битый/чёрный файл → regen-цель без vision; duration-
      ошибка не превращается в 8.0; приёмка отбраковывает.

## Гигиена

- [ ] H.1 Warning на старте при отсутствии каталогов промптов К2/К3
