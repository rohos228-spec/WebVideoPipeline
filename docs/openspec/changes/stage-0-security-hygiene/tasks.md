...
- [x] 1.5 PII из `HANDOVER.md` (chat_id владельца, имя бота) → отсылка к
      `TELEGRAM_OWNER_CHAT_ID` в `.env`; шапка-предупреждение об
      устаревании дока.
- [x] 1.6 Пересборка ai-pack отложена до чистки `.env.example`
      (иначе копия `ai-pack/03_config/env.example` затащит ключ обратно —
      инцидент пойман и откачен 2026-08-20). После п.1.2: `build_ai_pack.py`
      + `verify_ai_pack.py` + закоммитить копию.
