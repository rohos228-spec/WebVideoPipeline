...
- [ ] A.1 Ветка `merge/customer-fork` от `main`; remote `theirs` уже
      подключён, `theirs/main` = `17eff87d` (зафиксировать в PR).
- [ ] A.2 Снимок «до»: `git merge-tree --merge-base=3bbf5c31 main theirs/main`
...
- [ ] B.1 `git checkout theirs/main -- web/src web/public/favicon.svg
      web/public/icon.svg`; удалить их `web/src/app/page.tsx` (наш `/`
      остаётся), не брать `web/out`.
- [ ] B.2 Вернуть наши ours-only файлы (33, список §1.1 п.2) — они не
      тронуты checkout'ом, проверить `git status`.
- [ ] B.3 Наложить наши дельты на 9 общих файлов по таблице §1.1 п.3
...
- [ ] B.4 `globals.css`: их правки (шрифты, `--ring`) перенести в
      `studio-theme.css` под `body:has([data-studio-scope])`; `/` не
      перекрашивается. Проверить скриншотом `/` до/после.
- [ ] B.5 `lib/api.ts`: их клиент + наши граф-функции; `http()` — наш
...
- [ ] B.6 `node-model-catalog.ts`: kie-модели `online:false`; вкладка
...
- [ ] B.7 `orchestrator-panel.tsx`: дефолт свёрнут (наш комментарий).
- [ ] B.8 `app/pipeline/page.tsx`: сверить пропсы компонентов с их
      сигнатурами (Inspector, ProjectSidebar, StudioWorkspace,
      OutseeCreateWorkspace, GptWorkspace изменились); `?project=` /
      `?template=1` работают.
- [ ] B.9 `tsc --noEmit` и `next build` зелёные; `web/STUDIO_VERSION`
      — взять их (504…) с пометкой источника.
- [ ] B.10 Коммит с трейлером `Source: theirs/main 17eff87d`.

## C. Бэкенд под их фронт (§1.3, без kie)

- [ ] C.1 meta-agent: `routers/meta_agent.py`, `services/meta_prompt_compiler.py`,
...
- [ ] C.2 shot-menu: `db_browser.py` (+142), `services/shot_menu.py`,
...
- [ ] D.1 `gpt_api.py`: `store: False` + `_responses_reasoning_block`
      (0be45ae2) поверх нашего salvage.
- [ ] D.2 `outsee_http.py`: ожидание `concurrency_limit` (8b539721),
      `raise last_err` вынести из-под `while True`.
- [ ] D.3 `settings.py`: `env_file_encoding="utf-8-sig"`,
      `env_ignore_empty=True`; `.env.env` НЕ читать.
- [ ] D.4 ElevenLabs прямой TTS (052db0b1) — только по подтверждению,
      что TTS без CDP нужен; иначе пропустить.
- [ ] D.5 Не тащить: `/credits` fingerprint, VPS-роутинг kie,
      `KIE_VIDEO_IDS`×11, `_WORKING_KEY`-кэш ключей.

## E. Гейт и CI

- [ ] E.1 `ci.yml` «Никакого hex вне токенов»: храповик по файлам →
...
- [ ] E.2 `.gitleaksignore`: отпечатки на `node-model-catalog.ts`
      (`kling-*` ложные) и `baza-workspace.tsx` (уже есть) — по факту
      прогона `gitleaks detect --log-opts=main..HEAD`.
- [ ] E.3 `.gitignore`: убедиться, что `web/out` игнорируется, `.env.env`
      добавить (на будущее).
- [ ] E.4 Hex-гейт, tsc, next build, `pytest` api-surface — зелёные в CI.

## F. Смоук и приёмка

- [ ] F.1 Смоук на копии базы (`run_web.sh` + `smoke.mjs`, headless
...
- [ ] F.2 Скриншоты `/` и `/pipeline` до/после — `/` не изменился.
- [ ] F.3 Обновить `docs/PIPELINE-UI-MAP.md` (акт четвёртый: студия
      заказчика 2026-09) и память `studio-shell-restored`.
- [ ] F.4 Release через `main` → GHCR → VPS; проверка `rev` в контейнере
      и наличия новых чанков; заказчику — `/pipeline?project=N` с
      Ctrl+Shift+R.

## G. Оркестрация Kir0029 — ПОСЛЕ ревью §3.3

- [ ] G.1 Список полей ответов, которые читает их фронт и пишет только
      их реестр статусов/таймеры (`node_runs`, `started_at`,
      `finished_at`, бейджи результатов/ошибок).
- [ ] G.2 Решение: контракт ответа поверх нашего оркестратора (tool_use)
      или перенос реестра целиком. Задачи — по решению.

## H. Цепочка промтов — ПОСЛЕ ревью §3.4

- [ ] H.1 `prompts/05_excel_gpt/sd_*.md`: они удалили, мы используем
      (`sd_action_vlog`) — не трогать до вердикта.
- [ ] H.2 `scene_design/*`, `camera_expand`, `vo_shot_expand`,
      `img_pr`-батчи, `db_apply`-алиасы, montage AI change — по вердикту.
