...
> ⚠️ **Реестр отстал.** Сверка делалась на `3bbf5c3` (= `main`); ветка
> `hygiene-rebuild`/`hardening` ушла вперёд на 50+ коммитов. Что уже
> неверно в таблице ниже:
> - `HANDOVER.md` — переписан 2026-08-22 (56897a4), вердикт
>   «ПРОТИВОРЕЧИТ КОДУ» снят;
> - `docs/AGENT_MAP.md` — переписан там же;
> - `docs/DB_V2.md` — противоречие про `export_xlsx` устранено, код и док
>   сходятся (`db_apply.py:1048` `export_xlsx: bool = False`);
> - `AGENTS.md` — три расхождения (провайдер по умолчанию, `web/out` в git,
>   счётчики ruff/mypy) исправлены 2026-08-22.
> Живо: `import_project_xlsx` действительно не подключён ни к одному роуту.
> Перед следующим этапом — пересверить реестр целиком.
