...
фактической границе ответа около 94 000 обрезался трижды по ~12 минут и
прошёл с четвёртой попытки — штатное дробление пополам так и не сработало,
результат получен угадыванием на последней ступени лестницы ретраев. При
`GPT_MAX_RETRIES=3` тот же шаг ушёл бы в split, то есть исход зависел от
настройки ретраев, а не от плана.
...
#### Ответы владельца (2026-09-01) — решения приняты

1. **Каст: единственный писатель — узел `cast`.** Сцен-агент может только
   *предложить* правку, и она проходит `diff → показ → применение`, как
   правка графа. Дорогая ошибка ловится до генераций; цена — один шаг
   подтверждения в ролике. Вариант «агент пишет, `hero_descriptions` уходит»
   отклонён: каст определяла бы модель, а человек правил бы уже нарисованное.
2. **Пороги приёмки — пресетами в коде.** Точечная настройка на узле уже
   работает (`acceptance_profile`, коммит `391f391`), новый жанр = один
   коммит. Пресеты данными отклонены: приёмку смогли бы тихо отключить
   правкой JSON, а её ценность именно в том, что она не обсуждается.
3. **Промты: сначала хэш, потом база.** В граф кладётся хэш содержимого —
   дёшево, и перезапуск честно говорит «промт изменился с прошлого прогона».
   Перенос текстов в `prompt_versions` как источник правды — следующим шагом,
   вместе с редактором в интерфейсе.
